...
- skill 數量 + **所有 skill description 合計 bytes**(每 session 的固定租金:description 不管觸不觸發都常駐)
...
跟 `~/.claude/skills/` 實際安裝清單交叉比對,列出從未觸發的。記錄可能上千檔,給 timeout;plugin skill 名稱是 `plugin:skill` 格式,尾端 `sed` 負責正規化。
